...
- **V8 Engine:** JavaScript Engine that uses Just-In-Time (JIT) compiler to compile and translate JS to machine code. This engine also includes a garbage collector which cleans up old, unneeded data from the memory. This clean-up tool improves the engine's efficiency and performance because it frees up space that might slow down the engine.
...
- **Global Objects and APIs:** 
      ...
      *  Node.js replaces window with the global object. It uses the global object and built-in modules (fs, os, http) that can access the file system, operating system, and network to handle server-side operations. This allows Node.js to build REST APIs  by loading modules from the local filesystem and expose OS level functionality. Node has no access to the DOM, window object, or  document object.
...
According to the Node.js Foundation, the V8 engine (written in C++) is Google’s open-source JavaScript engine that parses, compiles, and executes JavaScript code within the Google Chrome browser. It can also parse, compile, and execute JavaScript code independent of the browser where it's hosted. (https://nodejs.org/en/learn/getting-started/the-v8-javascript-engine).

**Execution Power:** 

The V8 Engine uses an interpreter (Ignition) and JIT compiler (TurboFan) to process JavaScript code into machine code. The V8 can only parse, compile, execute JavaScript, but it does not provide browser-specific global objects(document, window,etc). V8 has a C++ API so it can be embedded in any C++ application, e.g. Node.js. The V8 engine includes a Garbage Collector that manages memory automatically, freeing up space by removing unneeded data and thus improving the speed and efficiency of the engine.

**Node integration of V8:** 

Node embeds V8 engine as its JavaScript runtime. Node.js adds V8’s capabilities to its own built-in modules (fs, os, http) and libuv library. Node’s built-in modules provide access to the local file system and operating system while libuv library assists Node.js with asynchronous I/O operations within local networks and filesystems. The combination of the V8 engine, built-in modules and libuv enable Node.js to provide server side capabilities by replacing browser-specific objects (such as DOM or a Web-API platform) with direct access to the local filesystem and network.
...
- **File System Management:**
  _ Reading and writing JavaScript code in a local filesystem
  _ Creating or removing directories
...
- **Server-Side Applications:** 
  _ Building web servers
  _ Building REST APIs
...
- **Access the Local Operating System:**
  _ Change permissions for a file
  _ Retrieve information about the operating system
